...
10. **Public OpenAPI = INSTALLER API, not resident** (discovered 2026-07-07 Gate B via live probing with real password-grant token). The `/flexMon/v1/openapi.json` we vendored describes the *installer/admin* API surface: it requires the `clientApiUser` OAuth2 client_credentials security scheme, which needs an API Account created in the SIMPL admin UI (**not exposed to residents**). Residents authenticated via password grant against the same Keycloak realm receive a strictly-restricted subset of endpoints. Empirical resident-scope coverage:
    - **200 OK (resident-accessible)**: `/installations` (list only), `/meters`, `/measurementtypes`, `/measurements`, `/communicators`, `/aggregationdayvalues`, `/aggregationmonthvalues`, `/aggregationyearvalues`, `/emsconfigurations`, `/realtimevalues` (this is the SPA's live-data endpoint, NOT `/instantvalues`), `/scheduledbatteryactions` (SPA's battery control path).
    - **403 Forbidden (installer-only)**: `/instantvalues`, `/batteries`, `/inverters`, `/hvacs`, `/alarms`, `/events`, `/controlsignals`, `/measuringdevices`, `GET /installations/{id}` (single-item — list works).
    - **415 Unsupported Media Type on empty POST** (endpoint exists, needs body): `POST /installations/{id}/activateContinuousProcessing` — residents CAN trigger realtime activation with the correct request body.
    - **HAL+JSON key casing**: multi-word `_embedded` keys are camelCase in real responses: `measurementTypes`, `aggregationDayValues`, `aggregationMonthValues`, `aggregationYearValues`, `realtimeValues`, `scheduledBatteryActions`, `emsConfigurations`. Single-word keys stay lowercase (`installations`, `meters`, `measurements`, `communicators`). Our hand-authored fixtures under-used camelCase and relied on the `_embedded_list` fallback via `additional_properties`; real API responses take the typed path directly when the fixtures are corrected.

    Decision (Gate B, 2026-07-07): **Hybrid mode**. Config flow adds an "API mode" selector (`resident` default, `installer`). Resident mode uses `/realtimevalues` in the instant coordinator (not `/instantvalues`), drops battery/inverter/HVAC/alarm platforms, keeps `/scheduledbatteryactions` as an optional battery control surface. Installer mode keeps the existing full-featured path but requires the user to obtain client_credentials from vendor support. Documented in this open question as authoritative reference for the mode split.

11. **HA shared httpx client integration** (deferred from P3 review 2026-07-07) — spec §4.2 mandates `homeassistant.helpers.httpx_client.get_async_client(hass)` for shared connection pool + proxy/cert inheritance, but `openapi-python-client`'s generated `AuthenticatedClient` creates its own private `httpx.AsyncClient` internally. `set_async_httpx_client()` exists to inject a shared instance, but binding the FlowBuddy Keycloak bearer via `client.auth=` on a truly shared HA client would leak the token into unrelated integrations' requests through that same client. Decision needed at P9.A implementation: (a) accept the deviation from §4.2 and document; (b) inject shared client + move Keycloak auth from client-level to per-request override; (c) bypass generated HTTP layer entirely. Interim: P3 ships with the generated client's private pool; `activate_continuous_processing` bypass path already uses the (currently unshared) `self._http`.
